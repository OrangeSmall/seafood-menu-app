...
def create_image(data_df, date_str, manual_upload=None):
    ...
    draw.text((margin, 50), "本週活體價格", fill=c_header_text, font=font_header)
    ...
    draw.text((margin, footer_y + 30), "Generated by SmallOrange seafood bot V5.0", fill="#CCCCCC", font=font_footer)
    return img
    ...
    with tab2:
        ...
        if selected_item and selected_spec:
            ...
            if not target_row.empty:
                # 取得歷史欄位 (日期欄位)
                # 排除固定欄位，剩下的就是日期
                dates = [c for c in df.columns if c not in fixed_cols and "Unnamed" not in c and c != ""]
                
                # 準備畫圖的數據
                chart_data = []
                for d in dates:
                    raw_price = str(target_row.iloc[0][d])
                    # 清洗數據：把 "$780" 變成 780.0，把 "售完" 變成 0
                    price_num = clean_price(raw_price)
                    if price_num > 0: # 只顯示有價格的日期，0或售完不顯示以免圖表亂掉
                        chart_data.append({"日期": d, "價格": price_num})
                
                if chart_data:
                    chart_df = pd.DataFrame(chart_data)
                    chart_df = chart_df.set_index("日期") # 將日期設為 X 軸
                    
                    st.write(f"**{selected_item} ({selected_spec}) 的價格走勢**")
                    
                    # 顯示直條圖
                    st.bar_chart(chart_df, color="#C19A6B") # 使用你的茶色系
                    
                    # 如果想要折線圖，把上面那行換成 st.line_chart(chart_df) 即可
                    
                    with st.expander("查看詳細數據表格"):
                        st.dataframe(chart_df.T) # 轉置顯示比較好看
                else:
                    st.warning("此品項目前沒有足夠的歷史價格數據可供繪圖。")
