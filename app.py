...
def create_image(data_df, date_str, manual_upload=None):
    ...
    for i, (name, group) in enumerate(grouped):
        ...
        if y_col1 <= y_col2:
            ...
            y_col2 += item_h
    # [V7.7] 為了在底部放 Logo，將底部留白從 100 增加到 150
    total_height = max(y_col1, y_col2) + 150 
    ...
    if watermark_source:
        try:
            ...
            r, g, b, a = wm.split()
            # 透明度設為 60% (0.60)，兼顧質感與清晰度
            a = a.point(lambda p: p * 0.60) 
            wm.putalpha(a)
            
            # 3. 計算置中位置 (水平置中，放置於底線下方 20px 處)
